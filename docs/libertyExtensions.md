...
| mergeServerEnv | boolean | 3.0 | Merge server.env properties from a server.env specified in the `configDirectory`, server.env file specified by the `serverEnvFile` property, or from in line properties specified in the `env` attribute with the generated server.env. Conflicts are resolved with the same precedence as the replacement policy when this property is `false`. A server.env in the `configDirectory` is of lowest precedence, a `serverEnvFile` being the next highest, and `env` inline being of highest precedence.  `false`. | No |
