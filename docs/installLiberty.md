...
| version | Exact or wildcard version of the WebSphere Liberty server to install. Available versions are listed in the [index.yml](http://public.dhe.ibm.com/ibmdl/export/pub/software/websphere/wasdev/downloads/wlp/index.yml) file. By default, the latest stable release is used if the Liberty repository is used. If 'archiveId' is specified to use  the Maven repository, the default version is '17.0.0.2'. Only used if `runtimeUrl` is not set. | No |
...
| type | Liberty runtime type to download from the Liberty or Maven repository. Currently, the following types are supported: `kernel`, `webProfile6`, `webProfile7`, and `javaee7`. Only used if `runtimeUrl` is not set. The default value is `webProfile6` for the Liberty repository and `webProfile7` for the Maven repository. | No |
...
2. Install Liberty runtime with all Java EE 7 features using Liberty repository.
  ```groovy
    apply plugin: 'liberty'
...
3. Install using Maven repository.
  ```groovy
    apply plugin: 'liberty'
...
4. Install from a specific location.
  ```groovy
    apply plugin: 'liberty'
    ...
    liberty {
        install {
            licenseCode = "<license code>"
            runtimeUrl = "<url to runtime.jar or zip>"
        }
    }
  ```
