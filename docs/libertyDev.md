...
| keepTempDockerfile | boolean | 3.1 | If set to `true`, dev mode will not delete the temporary file which is the Dockerfile used to build the container. This file is handy in case you need to debug the process of building the container. The name of the temporary file can be seen when dev mode displays the `docker build` command. The default value is `false`.| No |
