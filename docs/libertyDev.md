...
| skipDefaultPorts | boolean | 3.1-M3 (tech preview) | If set to `true`, dev mode will not publish the default Docker port mappings of `9080:9080` and `9443:9443`. Use this option if you would like to specify alternative local ports to map to the exposed container ports using the `dockerRunOpts` option. | No |
