...
| dockerTimeout | integer | 3.1-M2 (tech preview) | Specifies the amount of time dev mode shall wait for the completion of docker operations such as to build the container. The value is specified in seconds and may be between 1 and 3600 inclusive. The default value is 60 seconds. | No |