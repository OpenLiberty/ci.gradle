...
| dockerBuildTimeout | integer | 3.1-M2 (tech preview) | Maximum time to wait (in seconds) for the completion of the docker operation to build the container. The value must be an integer greater than 0. The default value is `60` seconds. | No |