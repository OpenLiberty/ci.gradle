...
class LibertyPackage_archiveJar_dirWithSpace_newPosixTest extends AbstractIntegrationTest{
    static File resourceDir = new File("build/resources/test/liberty-package-test")
    static File buildDir = new File(integTestDir, "/liberty-package-archiveJar-with-space-test-new-posix")
    static File packageDir = new File(buildDir, "/folder with space")

    static File buildFilename = new File(resourceDir, "liberty-package-archiveJar-dirWithSpace-posix.gradle")
    ...
    public void test_package_archiveJar() {
        try{
           ...
           def file = new File(packageDir, 'testPackage.jar')
