...
*/
    ...
    public void test_loose_config_file_contents_are_correct(){
      ...
      if (OSUtil.isWindows()) {
          Assert.assertTrue('archive sourceOnDisk attribute value not correct', nodeValue.endsWith("\\commons-text-1.14.0.jar") && !nodeValue.contains("\\test-loose-application-with-war-tasks\\"))
      } else {
          Assert.assertTrue('archive sourceOnDisk attribute value not correct', nodeValue.endsWith("/commons-text-1.14.0.jar") && !nodeValue.contains("/test-loose-application-with-war-tasks/"))
      }
