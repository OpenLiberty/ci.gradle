...
public class TestSpringBootApplication40 extends AbstractIntegrationTest{
    static File resourceDir = new File("build/resources/test/sample.springboot4")
    ...
    @Test
    public void test_spring_boot_apps_40() {
        try {
    ...
    @Test
    public void test_spring_boot_classifier_apps_40() {
        try {
            runTasks(buildDir, 'deploy', 'libertyStart')
    ...
    @Test
    public void test_spring_boot_classifier_apps_40_no_feature() {
        try {
    ...
    @Test
    public void test_spring_boot_war_apps_40() {
        try {
            runTasks(buildDir, 'deploy', 'libertyStart')
    ...
    @Test
    public void test_spring_boot_war_classifier_apps_40() {
        try {
            runTasks(buildDir, 'deploy', 'libertyStart')
    ...
    @Test
    public void test_spring_boot_dropins_40() {
        try {
            runTasks(buildDir, 'deploy', 'libertyStart')
            String webPage = new URL("http://localhost:9080").getText()
            Assert.assertEquals("Did not get expected http response.","Hello!", webPage)
    ...
    @Test
    public void test_spring_boot_plugins_dsl_apps_40() {
        try {
            runTasks(buildDir, 'deploy', 'libertyStart')
    ...
    @Test
    public void test_spring_boot_with_springbootapplication_apps_40() {
        try {
            runTasks(buildDir, 'deploy', 'libertyStart')
    ...
    @Test
    public void test_spring_boot_with_springbootapplication_nodes_apps_40() {
        try {
            BuildResult result = runTasksFailResult(buildDir, 'deploy', 'libertyStart')
            String output = result.getOutput()
            assertTrue(output.contains("Found multiple springBootApplication elements specified in the server configuration"))
        } catch (Exception e) {
            throw new AssertionError ("Fail on task deploy.", e)
        }
    }
    ...
    @Test
    public void test_spring_boot_with_springbootapplication_nodes_apps_include_40() {
        try {
            BuildResult result = runTasksFailResult(buildDir, 'deploy', 'libertyStart')
            String output = result.getOutput()
